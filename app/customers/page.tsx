...
function calcStats(customer: Customer): CustomerStats {
  ...
  const sortedCompleted = [...completed].sort((a, b) => b.start_at.localeCompare(a.start_at))

  return {
    totalCount: reservations.filter(r => r.status !== 'cancelled').length,
    completedCount: completed.length,
    cancelCount: cancelled.length,
    totalAmount: completed.reduce((s, r) => s + (r.price ?? 0), 0),
    lastVisit: sortedCompleted[0]?.start_at ?? null,
    byProduct,
    upcoming: reservations.filter(r => r.status === 'scheduled' && r.start_at >= now).sort((a,b)=>a.start_at.localeCompare(b.start_at)),
    past: reservations.filter(r => r.status !== 'scheduled' || r.start_at < now).sort((a,b)=>b.start_at.localeCompare(a.start_at)),
  }
}

/* ── 고객 상세 슬라이드업 ── */
function CustomerDetail({
  customer, onClose, onUpdate, onDelete,
}: {
  customer: Customer
  onClose: () => void
  onUpdate: (id: number, data: { name: string; phone: string; memo: string }) => Promise<void>
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-white rounded-t-3xl px-4 pt-5 pb-8 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex gap-1">
            {editing ? (
              <>
                ...
                <button onClick={handleSave} disabled={saving} className="px-3 py-1.5 rounded-lg text-xs text-white" style={{ background: '#ec4899' }}>
                  {saving ? '...' : '저장'}
                </button>
              </>
            ) : (
        ...
        {stats.lastVisit && (
          <p className="text-xs text-gray-400 mb-3">마지막 방문: {fmtDate(stats.lastVisit)}</p>
        )}
            ...
            <div className="space-y-1.5">
              {stats.past.slice(0, 10).map(r => (
                <div key={r.id} className="flex items-center gap-2 bg-gray-50 rounded-xl px-3 py-2">
                  <span
                    className="text-[10px] font-700 px-2 py-0.5 rounded-full shrink-0"
...
function CustomerForm({ onSave, onCancel }: {
  onSave: (form: { name: string; phone: string; memo: string }) => Promise<void>
  onCancel: () => void
}) {
    ...
    <div className="fixed inset-0 z-50 flex flex-col justify-end">
      ...
      <div className="relative bg-white rounded-t-3xl px-4 pt-5 pb-8 space-y-3">
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={handleSave} disabled={saving || !form.name.trim()} className="flex-1 py-2.5 rounded-xl text-sm font-700 text-white disabled:opacity-40" style={{ background: 'linear-gradient(135deg, #a855f7, #ec4899)' }}>
            {saving ? '저장 중...' : '저장'}
          </button>
        </div>
      </div>
...
export default function CustomersPage() {
  ...
  const handleAdd = async (form: { name: string; phone: string; memo: string }) => {
  ...
  const handleUpdate = async (id: number, data: { name: string; phone: string; memo: string }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="px-4 py-4 space-y-2">
        {loading ? (
          ...
          <div className="text-center py-14">
            ...
            <p className="text-sm font-600 text-gray-500">등록된 고객이 없습니다</p>
          </div>
        ) : (
          sorted.map(c => {
            const s = calcStats(c)
            return (
              <button
                key={c.id}
                onClick={() => setDetail(c)}
                className="w-full text-left bg-white rounded-2xl border border-gray-100 px-4 py-3 shadow-sm flex items-center gap-3"
              >
                ...
                {s.upcoming.length > 0 && (
                  <span className="text-[10px] font-700 text-pink-500 bg-pink-50 rounded-full px-2 py-0.5 shrink-0">
                    예약 {s.upcoming.length}
                  </span>
                )}
              </button>
            )
      ...
      {showForm && (
        <CustomerForm onSave={handleAdd} onCancel={() => setShowForm(false)} />
      )}
